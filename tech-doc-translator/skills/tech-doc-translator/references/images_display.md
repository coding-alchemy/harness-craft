...
- **解析期**：四类 `parse_*_html.py` 对每个进入输出的图片出现登记源显示宽度，写出 `<输出名>.images_display.json`。优先可解析的内联 CSS `width`，再 HTML `width` 属性（整数像素）；百分比必须找到祖先链中最近的内联像素宽度作参照才算确定。暗/亮图片对只登记选中版本。无宽度约束、不支持单位、百分比无参照分别列入 `undetermined`，不冒充已恢复。解析输出落入系统临时目录时醒目告警（真实路径与符号链接别名都识别，名字相似的普通目录不误报），调试仍可成功。
