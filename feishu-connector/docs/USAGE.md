...
本连接器向一个已配置 Open ID 发送消息：显式 `send` 为纯文本，`task` 为自包含 interactive Markdown 消息卡片。Codex 与 OpenCode 共用相同的 argv/stdin 显式接口。运行时仅需 Python 3.9+。
...
每个动态值必须是独立 argv，禁止动态 Shell 拼接。`task` 只接受 `--status success|failure|confirm`、非空 `--project`、非空 `--conversation`、非空 `--content` 与可选 `--auto`。项目名和对话框名必须是单行有效 Unicode，拒绝空值、换行和 NUL；正文必须是非空有效 Unicode，允许 Markdown 换行，并原样传递、不解析、重组、截断或拆分，即使超过飞书限制。
...
仅 Shell 执行器若有独立 stdin 通道，使用固定命令 `python3 "$ENTRY" stdin`。stdin 只允许以下两种白名单 JSON，动态值不能进入 Shell 命令字符串：
...
自动通知只有在 `task --auto` 和 `notification.autoNotify=true` 时发送；手动取消不调用连接器，每个最终结果或待确认节点最多通知一次。失败内容只能给用户可见原因，不能包含内部推理、完整日志或敏感上下文；通知错误不得改变原任务结果。保留配置、环境变量、退出码、重试与 UUID 合同；完整说明见 [README](../README.md)。
